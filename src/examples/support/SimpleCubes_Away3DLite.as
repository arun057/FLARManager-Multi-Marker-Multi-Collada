package examples.support {
	...
	public class SimpleCubes_Away3DLite extends Sprite {
		...
		public function SimpleCubes_Away3DLite (flarManager:FLARManager, viewportWidth:Number, viewportHeight:Number, stage:Stage) {
			this.bMirrorDisplay = flarManager.mirrorDisplay;
			
			this.init(flarManager.numLoadedPatterns);
			this.initAway3DEnvironment(
				flarManager.cameraParams,
				viewportWidth, viewportHeight, stage,
		...
		private function initAway3DEnvironment (cameraParams:FLARParam, viewportWidth:Number, viewportHeight:Number, stage:Stage, viewportToSourceWidthRatio:Number) :void {
			...
			this.camera3D = new FLARCamera3D(cameraParams, viewportToSourceWidthRatio);
		...
		private function updateCubes () :void {
			...
			while (i--) {
				...
				while (j--) {
					...
					container.transform.matrix3D = FLARGeomUtils.convertFLARMatrixToFlashMatrix3D(marker.transformMatrix, this.bMirrorDisplay);
				}
			}
		}
